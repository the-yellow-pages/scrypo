...
import { CreateProfileRequestBody, UpdateProfileRequestBody } from './types.ts'; // Import the interfaces
dotenv.config();
...
const validateProfileData = (data: CreateProfileRequestBody | UpdateProfileRequestBody, isUpdate: boolean = false): string[] => {
    const errors: string[] = [];
    ...
    if (!isUpdate) {
        ...
        if (!createData.address || typeof createData.address !== 'string' || createData.address.trim() === '') {
    ...
    // Tags validation
    for (let i = 0; i < 4; i++) {
        const tagField = `tags${i}` as keyof (CreateProfileRequestBody | UpdateProfileRequestBody);
    ...
    if (!isUpdate) { // Create mode: location is required as per schema (notNull)
        ...
        const createData = data as CreateProfileRequestBody;
        if (!createData.location || typeof createData.location !== 'object' || createData.location === null ||
            typeof createData.location.x !== 'number' || typeof createData.location.y !== 'number') {
        ...
        if (updateData.hasOwnProperty('location')) {
            if (updateData.location === null || typeof updateData.location !== 'object' ||
                typeof updateData.location.x !== 'number' || typeof updateData.location.y !== 'number') {
    ...
    for (const field of pubkeyFields) {
        if (data.hasOwnProperty(field)) {
            const fieldValue = data[field as keyof (CreateProfileRequestBody | UpdateProfileRequestBody)];
...
export const createProfile = async (req: express.Request, res: express.Response) => {
    try {
        const newProfile: CreateProfileRequestBody = req.body;
        ...
        const profileToInsert = {
            ...newProfile,
            tags0: newProfile.tags0 !== undefined && newProfile.tags0 !== null ? String(newProfile.tags0) : null,
            tags1: newProfile.tags1 !== undefined && newProfile.tags1 !== null ? String(newProfile.tags1) : null,
            tags2: newProfile.tags2 !== undefined && newProfile.tags2 !== null ? String(newProfile.tags2) : null,
            tags3: newProfile.tags3 !== undefined && newProfile.tags3 !== null ? String(newProfile.tags3) : null,
            pubkey_hi: newProfile.pubkey_hi !== undefined && newProfile.pubkey_hi !== null ? String(newProfile.pubkey_hi) : null,
            pubkey_lo: newProfile.pubkey_lo !== undefined && newProfile.pubkey_lo !== null ? String(newProfile.pubkey_lo) : null
        };

        const result = await db.insert(schema.profiles).values(profileToInsert).returning();
...
export const getProfilesWithinArea = async (req: express.Request, res: express.Response) => {
    try {
        const { x1, y1, x2, y2 } = req.query;

        if (x1 === undefined || y1 === undefined || x2 === undefined || y2 === undefined) {
            res.status(400).json({ error: 'Missing query parameters: x1, y1, x2, y2 are required.' });
            return;
        }
        ...
        if (isNaN(nX1) || isNaN(nY1) || isNaN(nX2) || isNaN(nY2)) {
            res.status(400).json({ error: 'Invalid query parameters: x1, y1, x2, y2 must be numbers.' });
            return;
        }
        ...
        console.error(error);
        res.status(500).json({ error: 'Failed to retrieve profiles within area' });
    }
};
...
export const updateProfile = async (req: express.Request, res: express.Response) => {
    try {
        const { address } = req.params;
        const updatedFieldsBody: UpdateProfileRequestBody = req.body;
        ...
        if (updatedFieldsBody.pubkey_hi !== undefined) {
            fieldsToUpdate.pubkey_hi = updatedFieldsBody.pubkey_hi !== null ? String(updatedFieldsBody.pubkey_hi) : null;
        }
        if (updatedFieldsBody.pubkey_lo !== undefined) {
            fieldsToUpdate.pubkey_lo = updatedFieldsBody.pubkey_lo !== null ? String(updatedFieldsBody.pubkey_lo) : null;
        }
        if (updatedFieldsBody.tags0 !== undefined) {
            fieldsToUpdate.tags0 = updatedFieldsBody.tags0 !== null ? String(updatedFieldsBody.tags0) : null;
        }
        if (updatedFieldsBody.tags1 !== undefined) {
            fieldsToUpdate.tags1 = updatedFieldsBody.tags1 !== null ? String(updatedFieldsBody.tags1) : null;
        }
        if (updatedFieldsBody.tags2 !== undefined) {
            fieldsToUpdate.tags2 = updatedFieldsBody.tags2 !== null ? String(updatedFieldsBody.tags2) : null;
        }
        if (updatedFieldsBody.tags3 !== undefined) {
            fieldsToUpdate.tags3 = updatedFieldsBody.tags3 !== null ? String(updatedFieldsBody.tags3) : null;
        }
